<section class="slider-home">
    <div class="container">
        <div class="col-xs-12 col-md-12 col-mg-8 col-lg-8">
            <h1 class="page-title">Want to join the movement?</h1>
            <!-- <p class="readable-column"><em>Subscribe now to join the movement.</em></p> -->
            <p class="readable-column"><em>Subscribe now and be first to hear about Heddoko product announcements. <br>We'll never spam you.</em></p>
            <!-- Begin MailChimp Signup Form -->
            <div id="mc_embed_signup">
                <form class="form" action="http://heddoko.us8.list-manage.com/subscribe/post-json?u=4752cc710a498b9b75d697500&amp;id=b22a426cab&amp;c=?" method="get" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" novalidate="">
                    <div id="mc_embed_signup_scroll">
                        <div class="mc-field-group form-group">
                            <input type="text" class="form-control" value="" name="FNAME" class="form-control" id="mce-FNAME" placeholder="Full Name">
                        </div>
                        <div class="mc-field-group form-group">
                            <input type="email" value="" name="EMAIL" class="required email form-control" id="mce-EMAIL" placeholder="Email Address">
                        </div>
                        <div class="mc-field-group form-group">
                            <select name="MMERGE3" class="" id="mce-MMERGE3">
                                <option value="" disabled selected>Profession</option>
                                <option value="Ergonomics Professional">Ergonomics Professional</option>
                                <option value="Health Professional">Health Professional</option>
                                <option value="Occupational Safety Professional">Occupational Safety Professional</option>
                                <option value="Coach">Coach</option>
                                <option value="Athlete">Athlete</option>
                                <option value="Other">Other</option>
                            </select>
                        </div>
                        <div id="mce-responses" class="clear">
                            <div class="response" id="mce-error-response" style="display:none"></div>
                            <div class="response" id="mce-success-response" style="display:none"></div>
                        </div>
                        <!-- real people should not fill this in and expect good things - do not remove this or risk form bot signups-->
                        <div style="position: absolute; left: -5000px;">
                            <input type="text" name="b_4752cc710a498b9b75d697500_b22a426cab" tabindex="-1" value="">
                        </div>
                        <div class="clear form-group">
                            <input type="submit" value="Subscribe" name="subscribe" id="mc-embedded-subscribe" class="button btn btn-default btn-block">
                        </div>
                        <div id="thankyoudiv">
                            <h3>Thank You!</h3></div>
                        <img src="../images/ajax-loader.gif" class="ajax">
                    </div>
                </form>
            </div>
            <!--End mc_embed_signup-->
        </div>
    </div>
</section>